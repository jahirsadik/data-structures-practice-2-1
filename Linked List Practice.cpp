...
// Using stack to find out if it's palindrome

bool isPalindromeUsingStack(){
  node *current_node = head;
  stack <int> stck;
  while(current_node != NULL){
    stck.push(current_node->data);
    current_node = current_node->next;
  }
  current_node = head;
  while(current_node!=NULL){
    int i = stck.top();
    stck.pop();
    if(current_node->data != i) return false;
    current_node = current_node->next;
  }
  return true;
}
...
//THIS IS ANOTHER EXAMPLE OF THE THIRD function
  if(ll.isPalindromeUsingStack()) printf("The given Linked List is now a palindrome\n");
  else printf("The given Linked List is NOT a palindrome\n");
