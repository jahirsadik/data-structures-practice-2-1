...
int main(){
    int numOfQueries, numOfComputers;
    string tempStr;
    scanf("%d", &numOfQueries);

    for(int tc =1; tc<=numOfQueries; tc++){
      scanf("\n%d\n", &numOfComputers);
      int count1 = 0, count2 = 0;
      DisjointSet obj(numOfComputers);

      while (1){
        if (!getline(std::cin, tempStr) || tempStr.empty()) break;
        char ch;
        int value1, value2;
        sscanf(tempStr.c_str(), "%c %d %d", &ch, &value1, &value2);
        if(ch == 'c'){
          obj.Union(value1-1, value2-1);  
        }
        else{
          if(obj.find(value1-1) == obj.find(value2-1)){
             count1++;
          }
          else{
             count2++;
